...
const template: Template = {
  ...
  defaults: {
    colors: { background:"#fff5e6", surface:"#ffffff", text:"#0a0a0a", muted:"#666", border:"#0a0a0a", accent:"#ff5c39", accentText:"#ffffff" },
    typography: { headingFont:"space-grotesk", bodyFont:"inter", scale:1, headingWeight:900, tracking:"tight", radius:12 },
...
${config.projects
  .map(
    (p, i) => `<div class="card" data-anim data-hover style="--i:${i}"><h3>${esc(p.title)}</h3><p>${esc(p.description)}</p><div>${p.tags.map((t) => `<em>${esc(t)}</em>`).join("")}</div>${p.url ? `<div style="margin-top:12px"><a href="${esc(p.url)}">View →</a></div>` : ""}</div>`,
  )
  .join("")}
</div>
<h2>Skills</h2>
...
.hero{font-size:clamp(48px,9vw,140px);font-weight:900;letter-spacing:-0.04em;line-height:0.9;margin:0 0 32px;font-family:var(--font-heading)}
...
.card{background:var(--surface);padding:24px;border-radius:var(--radius);box-shadow:0 2px 0 var(--border)}
.card h3{margin:0 0 8px;font-size:22px}
.card em{background:var(--accent);color:var(--accent-text);padding:2px 8px;border-radius:99px;font-size:11px;font-style:normal;margin-right:6px}
a{color:var(--accent);font-weight:600}
h2{font-size:36px;margin:48px 0 16px;font-weight:900;letter-spacing:-0.02em}
.tag{display:inline-block;padding:6px 12px;border:1.5px solid var(--border);border-radius:99px;margin:4px 6px 4px 0;font-size:13px;font-weight:600}
`, body);
  },
};
